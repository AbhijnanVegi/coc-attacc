...
from game.utils import check_collision, get_nearest_object, get_distance, get_nearest_pos, check_inside
...
class Barbarian(Attacker):
    def __init__(self, game, position: tuple):
        obj = np.array([[Back.YELLOW + "^" + Fore.RESET + Back.RESET]], dtype="object")
        super().__init__(
            game, position, obj, BARB_HP, BARB_DAMAGE, BARB_RATE, BARB_SPEED
        )

    def update(self):
        nearest = get_nearest_object(self.game.buildings, self.position)
        if not object:
            return super().update()
        ...
        if nearest is not None:
            if not check_collision(self, nearest):
    ...
    def _move(self, position: tuple):
        if (time.time() - self.last_move) < self.speed * self.game.effects["speed"]:
            return
        new_pos_X = (
            self.position[0] + np.sign(position[0] - self.position[0])
        )
        new_pos_Y = (
            self.position[1] + np.sign(position[1] - self.position[1])
        )
        self.last_move = time.time()
...
class King(Attacker):
    def __init__(self, game, position):
        obj = np.array([
            [Back.YELLOW + "/" + Fore.RESET + Back.RESET, Back.YELLOW + "\\" + Fore.RESET + Back.RESET],
            [Back.YELLOW + '\\' + Fore.RESET + Back.RESET, Back.YELLOW + "/" + Fore.RESET + Back.RESET],
        ], dtype="object")
        super().__init__(game, position, obj, KING_HP, KING_DAMAGE, KING_RATE, KING_SPEED)
    ...
    def _attack(self):
        if (time.time() - self.last_attack) > self.rate:
            ...
            for building in (self.game.buildings + self.game.walls):
                ...
                if get_distance((self.position[0]+self.size[0]/2,self.position[1]+self.size[1]/2),nearest) < self.range:
